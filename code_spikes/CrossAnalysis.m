...
for anm = [2 3 5]
    ...
    xm_all = cell(n_gammas,n_groups);
    cm_all = cell(n_gammas,n_groups);
for sess = 2:3
...
GammaPow_U = smoothdata(GammaPow_U, 'gaussian',50);
...
for dir =1:2
    ...
    if dir ==1
     ...
     dataspikes_1 = spikes(p_cells);
     dataspikes_2 = spikes(p_cells);
    ...
    cc_all = zeros(201,n_gammas,numel(dataspikes_1),numel(dataspikes_2));
    
    
for ii=1:numel(dataspikes_1)

    

           SpikeVec = dataspikes_1{1, ii}.t;    

           

           PF_Limits_1 = pf_limits_1(ii,:);
           
           
           if numel(SpikeVec)<20 || PF_Limits_1(2)>=90 || PF_Limits_1(2)<=10
               continue
           end
...
SpikeVec_1 = SpikeVec;
    ...
    for jj = 1:numel(dataspikes_2)


        
           SpikeVec = dataspikes_2{1, jj}.t;   

           PF_Limits_2 = pf_limits_2(jj,:);

            
           if numel(SpikeVec)<20 || PF_Limits_2(2)>=90 || PF_Limits_2(2)<=10
               continue
           end
           
           D1 = [diff(SpikeVec(1:2)) ; min(diff(SpikeVec(1:end-1)),diff(SpikeVec(2:end))); diff(SpikeVec(end-1:end))];
D1(D1<0) = 1000;
spikes_bursts_2 = SpikeVec(D1<0.009);
spikes_single_2 = SpikeVec(D1>0.009);
% 
...
% 
takespike = find(GammaPow_R(round(d2*1000))>Pbound(pp) & GammaPow_R(round(d2*1000))<Pbound(pp+1) & Spike_Pos_2>=PF_Limits_2(1) & Spike_Pos_2<=PF_Limits_2(3));
%takespike = find(Spike_Pos_2>=PF_Limits_2(1) & Spike_Pos_2<=PF_Limits_2(3));
data_2 = d2(takespike)*1000;




% takespike = find(GammaPow_R(round(d1*1000))>Pbound(pp) & GammaPow_R(round(d1*1000))<Pbound(pp+1));
% data_1 = d1(takespike)*1000;
% 
% takespike = find(d2);
% data_2 = d2(takespike)*1000;
...
cc = histcounts(DD2,linspace(-2*pi,2*pi,202),'Normalization','count');

    

cc = smoothdata(cc,'gaussian',15);
...
data_R = [];
for rr = 1:30
data_R = cat(1,data_R,data_2 + rand(size(data_2))*j_t - j_t/2);
end

data_2 = data_R;


DD = pdist2(data_1,data_2,@(x,y) y-x);

DD = DD(:);

%subplot(numel(Pbound)-1,2,2+(kk-1)*2)
cc_R = histcounts(DD,-201:2:201,'Normalization','probability');
        if(jj==ii)
            cc_R(51)=0;
        end

cc_R = smoothdata(cc_R,'gaussian',9);
...
cm(1)=(sum(cc(101:end))-sum(cc(1:101)))/(sum(cc(101:end))+sum(cc(1:101)));
cm(2)=(sum(cc(51:end-30))-sum(cc(30:51)))/(sum(cc(51:end-30))+sum(cc(30:51)));



%     c_f = cc;
%     
%     TF = find(islocalmax(c_f,'MaxNumExtrema',6));
%     [~,mTF]=min(abs(TF-51));
%     
%     
%     cm(2) = c_f(TF(mTF));
...
cm_all{pp,gro} = cat(1,cm_all{pp,gro},cm);
...
save([save_dir 'CCDati/CCDati_4GBothG_A' num2str(anm) 'S' num2str(sess) 'D' num2str(dir) 'Ga' num2str(comp) 'Sp' num2str(spike_type) 'Ph' num2str(ph_type)],'cc_all')
...
% if jitter == 0 
% save(['~/Dropbox/Projects_NIJ/Matteo/Nijmegen/CrossAnalysis_A' num2str(anm) '_Ty' num2str(spike_type) '_Ph' num2str(ph_type)],'xm_all','cm_all')
% else
%     save(['~/Dropbox/Projects_NIJ/Matteo/Nijmegen/CrossAnalysis_A' num2str(anm) '_Ty' num2str(spike_type) 'J'],'xm_all','cm_all')
% end
